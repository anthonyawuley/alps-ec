...
public class FSALPSGPInitializer extends GPInitializer
{
			...
			final Parameter base)
	{
		...
		// Load our constraints: default values taken from ec.gp.koza.koza.params
		for (int y=0;y<x;y++)
		{ 
			GPNodeConstraints c;
			...
			final Parameter base)
	{
		...
		for (int y=0;y<x;y++)
		{
			FSALPSGPFunctionSet c;
			// Figure the GPFunctionSet class
			if (state.parameters.exists(base.push(""+y), null))
				c = (FSALPSGPFunctionSet)(state.parameters.getInstanceForParameterEq(
						base.push(""+y),null,FSALPSGPFunctionSet.class));
			else
			{
				state.output.message("No GPFunctionSet specified, assuming the default class: ec.gp.GPFunctionSet for " + base.push(""+y));
				c = new FSALPSGPFunctionSet();
			}
			c.setup(state,base.push(""+y));
		}
	}
