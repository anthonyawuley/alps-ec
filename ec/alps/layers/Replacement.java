...
public abstract class Replacement extends TournamentSelection {
    ...
    /**
     * @return returns the name of a sub-class
     */
    @Override
     ...
     *
     * @param end    total size of pop for sub population
     * @param thread thread
     * @author Anthony
     */
    public void consolidatePopulation(ALPSLayers alps, final int thread) {
        ...
        for (int sub = 0; sub < alps.layers.get(alps.index).evolutionState.population.subpops.length; sub++) {
            ...
            } else {   /* consolidate current and next population */
     ...
     *
     * @param start  begin random copy of existing individuals
     * @param end    total size of pop for sub population
     * @param sub    supbpopulation
