...
public class GPIndividual extends Individual {
    ...
    /**
     * find frequency count for terninals/terninal sets in an individual
     */
    public void gatherIndividualNodeStats(final EvolutionState state, Map<String, Double> nodeCount) {
        for (int x = 0; x < trees.length; x++)
            trees[x].treeStats(state, nodeCount);
    }
     ...
     * log tree depth
     */
    public void individualTreeDepth(final EvolutionState state, final int log) {
        for (int x = 0; x < trees.length; x++)
            state.output.println("Tree Depth: " + trees[x].treeDepth(state), log);
    ...
    public void printTerminalCount(final EvolutionState state, Map<String, Double> map, final int log) {
        this.gatherIndividualNodeStats(state, state.nodeCountTerminalSet);

        state.output.println("Terminal Stats : ", log);
        for (Entry<String, Double> entry : map.entrySet())
            state.output.print("\t" + entry.getValue(), log);
        state.output.println(" ", log); //append next line
    }
