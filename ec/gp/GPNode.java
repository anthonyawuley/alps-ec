...
public abstract class GPNode implements GPNodeParent, Prototype {
    ...
    /**    */
    public int nodeFrequency = 0;
    /**    */
    public double nodeProbability = 0.0;
     ...
     * @author Anthony Awuley
     */
    public int nodeStats(final EvolutionState state,
                         final int verbosity, Map<String, Double> nodeCount) {
        String n = toString();
        //if(flag)
        //	state.output.print(n,log);

        /**
         * increment count of variable usage in population

         if(this.expectedChildren()==0)
         {
         double count = state.nodeCountTerminalSet.get(this.getClass().toString());
         nodeCount.put(this.getClass().toString(), count+1);
         //state.nodeCountTerminalSet.put(this.getClass().toString(), count+1);
         //System.out.println("layer:"+state.alps.index + " "+this.getClass().toString() +" : "+state.nodeCountTerminalSet.get(this.getClass().toString()));

         //double count = Engine.nodeCountTerminalSet.get(this.getClass().toString());
         //Engine.nodeCountTerminalSet.put(this.getClass().toString(), count+1);
         }
         */

        if (nodeCount.containsKey(this.getClass().getName())) {
            double count = state.nodeCountTerminalSet.get(this.getClass().getName());
            nodeCount.put(this.getClass().getName(), count + 1);
        }
    ...
    /**
     * recursively count a tree depth
     *
     * @param state
     * @return
     * @author Anthony Awuley
     */
    public int maxTreeDepth(final EvolutionState state) {
        int depth = 0;
        for (int x = 0; x < children.length; x++)
            depth = Math.max(depth, children[x].maxTreeDepth(state));

        return (children.length > 0) ? depth += 1 : 0;
    }
     ...
     * @author Anthony Awuley
     */
    public int nodeRootedTreeStats(final EvolutionState state,
                                   int nodes,
        ...
        nodes += nodeStats(state, Output.V_VERBOSE, nodeCount);

        for (int x = 0; x < children.length; x++)
            nodes = children[x].nodeRootedTreeStats(state, nodes, nodeCount);
