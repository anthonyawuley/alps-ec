...
public class GPTree implements GPNodeParent, Prototype {
    ...
    /**
     * count up to the maximum tree depth
     *
     * @param state
     * @return
     */
    public int treeDepth(final EvolutionState state) {
        return child.maxTreeDepth(state);
    }
     ...
     * @param log
     * @param verbosity
     * @param flag      return number of nodes in a tree : not confirmed
     * @author Anthony Awuley
     */
    public int treeStats(final EvolutionState state, Map<String, Double> nodeCount) {
        return child.nodeRootedTreeStats(state, 0, nodeCount);
    }
