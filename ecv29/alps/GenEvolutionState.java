...
public class GenEvolutionState extends EvolutionState
{
	...
	public void startFresh(Layer l) 
	{
		 ...
		 * always flags an error because replacement parameters are not set in
		 * the parameter file
		 */
		...
		int popSize = 0;
		for (int sub=0; sub < population.subpops.size(); sub++) {
			//popSize += population.subpops.get(sub).individuals.size(); 
			popSize += population.subpops.get(sub).initialSize;
		}
		...
		ArrayList<Individual> ind = new ArrayList<Individual>(Arrays.asList(new Individual[0]));
		this.population.subpops.get(0).individuals = ind;
	...
	public int evolve()
	{
		...
		Engine.globalEvaluations += population.subpops.get(0).initialSize;

		// SHOULD WE QUIT?
		// added last layer condition so ideal individuals migrate up layers
		if (evaluator.runComplete(this) != null && quitOnRunComplete && Engine.alps.index == Engine.alps.layers.size()-1)
		{
		...
		// SHOULD WE QUIT? -- 
		if (Engine.globalEvaluations >= Engine.alpsEvaluations && generation > numGenerations) {
			return R_FAILURE;
		}
		...
		// Increment ages
		for (Subpopulation s : population.subpops) {
		...
		// Added because individuals were not migrating between layers.
		replacement.layerMigrations(Engine.alps,population);
