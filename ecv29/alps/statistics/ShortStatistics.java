...
import ec.alps.GenEvolutionState;
...
public class ShortStatistics extends Statistics
{
	...
	public void postInitializationStatistics(final EvolutionState state)
	{
		...
		int firstGenOfLayer = ((GenEvolutionState)state).layer.getIsBottomLayer()?0:(((GenEvolutionState)state).layer.getMaxAge()/2)+1;

		if (output) state.output.print("" + firstGenOfLayer +  " ", statisticslog);
	...
	public void postEvaluationStatistics(final EvolutionState state)
	{
		...
		{       
			minimumAgeThisGen[x] = (long) state.population.subpops.get(x).individuals.get(0).age;
			maximumAgeThisGen[x] = (long) state.population.subpops.get(x).individuals.get(0).age;
			...
			for(int y=0; y<state.population.subpops.get(x).individuals.size(); y++)
			{
				if (state.population.subpops.get(x).individuals.get(y).evaluated)               // he's got a valid fitness
				{
					 ...
					 * @author anthony
					 */
					minimumAgeThisGen[x] = (long) Math.min(minimumAgeThisGen[x], state.population.subpops.get(x).individuals.get(y).age);
					maximumAgeThisGen[x] = (long) Math.max(maximumAgeThisGen[x], state.population.subpops.get(x).individuals.get(y).age);		   
				
					totalAgeThisGen[x] +=  state.population.subpops.get(x).individuals.get(y).age;
