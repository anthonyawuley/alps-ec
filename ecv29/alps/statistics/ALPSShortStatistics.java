			...
			for(int index=0;index< Engine.alps.layers.size() ;index++)
			{
				try
				{
					minimumAgeThisGen[x] = (long) Engine.alps.layers.get(index).evolutionState.
							population.subpops.get(x).individuals.get(0).age;
					...
					for(int y=0; y<Engine.alps.layers.get(index).evolutionState
							.population.subpops.get(x).individuals.size(); y++)
					{
						if (Engine.alps.layers.get(index).evolutionState.population.subpops.get(x).individuals.get(y).evaluated)               // he's got a valid fitness
						{
							// update sizes
							long size = Engine.alps.layers.get(index).evolutionState.population.subpops.get(x).individuals.get(y).size();
							...
							if (bestOfGeneration[x]==null ||
									Engine.alps.layers.get(index).evolutionState.population.subpops.get(x).individuals.get(y).fitness.betterThan(bestOfGeneration[x].fitness))
							{
								bestOfGeneration[x] = Engine.alps.layers.get(index).evolutionState.population.subpops.get(x).individuals.get(y);
							...
							/*@author anthony */
							minimumAgeThisGen[x] = (long) Math.min(minimumAgeThisGen[x], Engine.alps.layers.get(index).evolutionState.population.subpops.get(x).individuals.get(y).age);
							maximumAgeThisGen[x] = (long) Math.max(maximumAgeThisGen[x], Engine.alps.layers.get(index).evolutionState.population.subpops.get(x).individuals.get(y).age);		   

							totalAgeThisGen[x] +=  Engine.alps.layers.get(index).evolutionState.population.subpops.get(x).individuals.get(y).age;

							// sum up mean fitness for population
							totalFitnessThisGen[x] += Engine.alps.layers.get(index).evolutionState.population.subpops.get(x).individuals.get(y).fitness.fitness();
