    ...
    */
	...
	protected void loadElites(EvolutionState state, Population newpop)
	{
		...
		for(int x=0;x<state.population.subpops.size();x++)
		{
			 ...
			 * @author anthony
			 */
			if(state.population.subpops.get(x).individuals.size() < numElites(state, x))
			{
				elite[x] = state.population.subpops.get(x).individuals.size();	
			}
			else
			{ //original condition
		...
		for(int sub=0;sub<state.population.subpops.size();sub++) 
		{
			...
			if (numElites(state, sub)==1)
			{
				 ...
				 * @author anthony awuley
				 */
				if(state.generation != inds.get(inds.size()-1).generationCount/*!oldinds[best].parentFlag*/) 
				{ 
					inds.get(inds.size()-1).age++;
					inds.get(inds.size()-1).generationCount = state.generation;
				} 
			...
			else if (numElites(state, sub)>0)  // we'll need to sort
			{
				...
				for(int x=0;x<state.population.subpops.get(sub).individuals.size();x++) 
					orderedPop[x] = x;
				...
				for(int x=oldinds.size()-numElites(state, sub);x<oldinds.size();x++)
				{
					 ...
					 * @author anthony awuley
					 */
					// if(state.generation != inds.get(x).generationCount/*!oldinds[orderedPop[x]].parentFlag*/) 
					// { 
					// 	inds.get(x).age++;
					// 	inds.get(x).generationCount = state.generation;
					// } 
					if (state.generation != inds.get(inds.size()-1).generationCount) {
						inds.get(inds.size()-1).age++;
						inds.get(inds.size()-1).generationCount = state.generation;
					}
				}
