...
public abstract class Replacement extends TournamentSelection {
	...
	public void  consolidatePopulation(ALPSLayers alps,final int thread)
	{
		...
		for(int sub=0;sub<alps.layers.get(alps.index).evolutionState.population.subpops.size();sub++)
		{
			...
			{
				alps.layers.get(alps.index).evolutionState.population.subpops.get(sub).individuals =
						fillPopTournament(alps.layers.get(alps.index).evolutionState.population.subpops.get(sub).individuals.size(),
			...
			{   /* consolidate current and next population */
				for(int index=alps.index;index<alps.index+2;index++)
					alps.layers.get(index).evolutionState.population.subpops.get(sub).individuals =
					fillPopTournament(alps.layers.get(index).evolutionState.population.subpops.get(sub).individuals.size(),
			...
			final int thread) 
	{
		...
		for (int x = 1; x < state.population.subpops.get(subpopulation).individuals.size(); x++) {
	 ...
	 */
	public ArrayList<Individual> fillPopRandom(final int start, final int end,
			final int sub, final EvolutionState state, final int thread) 
	{
		...
		if (start == 0) // this condition implies the current population is empty.
			return state.population.subpops.get(sub).individuals;
		...
		for (int x = 0; x < (end - start); x++)
			inds[x] = (Individual) state.population.subpops.get(sub).individuals.get(getRandomIndividual(
		...
		for (int i = 0; i < end; i++)
			if (i < start)
				newInds[i] = state.population.subpops.get(sub).individuals.get(i);
			else
				newInds[i] = inds[count++];

		return new ArrayList<Individual>(Arrays.asList(newInds));
	}
	 ...
	 * @return population with filled in individuals to size of end
	 */
	public ArrayList<Individual> fillPopTournament(final int start, final int end,
			final int sub, final EvolutionState state, final int thread) 
	{
		Individual[] inds = new Individual[end - start];
		Individual[] newInds = new Individual[end];
		 ...
		 */
		for (int x = 0; x < (end - start); x++)
			inds[x] = (Individual) state.population.subpops.get(sub).
			individuals.get(bestTournament(sub, state, thread)).clone();
			...
			final EvolutionState state, final int thread) 
	{
		...
		for (int x = 1; x < s; x++) {
			int j = getRandomIndividual(x, subpopulation, state, thread);
			if (!betterThan(oldinds.get(j), oldinds.get(worst), subpopulation, state,thread)) // j is at least as bad as best
				worst = j;
		}
			...
			final EvolutionState state, final int thread) 
	{
		...
		for (int x = 1; x < s; x++) {
			int j = getRandomIndividual(x, subpopulation, state, thread);
			if (betterThan(oldinds.get(j), oldinds.get(best), subpopulation, state,
			...
			final Individual newIndividual) 
	{
		...
		for (int x = 1; x < s; x++) {
			int j = getRandomIndividual(x, subpopulation, state, thread);
			if (Math.abs(oldinds.get(nearest).fitness.fitness() - fitness) > Math
					.abs(oldinds.get(j).fitness.fitness() - fitness))
				nearest = j;
		}
